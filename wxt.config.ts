 ...
 * it. @mlc-ai/web-llm is additionally dynamic-import()ed at its use sites
 * (lib/webllm.ts, setup page) so it never lands in an initial chunk.
 * Same for @wllama/wllama (lib/wllama.ts, Firefox builds).
 */
...
export default defineConfig({
  vite: () => ({
    plugins: [tailwindcss() as any, vendorChunkGuard()],
    build: {
      ...
      // instead: @mlc-ai/web-llm / @wllama/wllama load lazily in
      // lib/webllm.ts + lib/wllama.ts + setup page, so the pages build emits
      // them as their own async chunks automatically.)
  ...
  // hosted in a hidden background-page iframe, see background/utils.ts).
